package s.com.eoi.util.eoiCollection
trait Tree[+A] {
      ...
      case true =>
        ...
        bs.filter(_.nonEmpty).headOption match {
          case some: Some[Tree[A]] =>
            Branch(some.get.value, bs.map(t => t.deleteNode(_ == some.get.value)))
          case None =>
            EmptyTree
        }
      case false =>
...
object Tree {
  ...
  private def insertChild[B >: A, A](root: Tree[A], p: B => Boolean, value: B): Tree[B] = root match {
    //满足条件的节点插入子节点 不可变结构
